...
class TestRTCBase:
    ...
    def test_validate_url(self):
        for idx, test_url in enumerate(TestRTCBase.test_urls):
            test_rtc = BaseTestRTC(test_url)
            assert (test_rtc.validate_url(test_url) ==
                    TestRTCBase.valid_test_urls[idx])
    ...
    def test_str(self):
        for test_url in TestRTCBase.test_urls:
            test_rtc = BaseTestRTC(test_url)
    ...
    def test_getattr(self):
        for idx, test_url in enumerate(TestRTCBase.test_urls):
            test_rtc = BaseTestRTC(test_url)
            assert test_rtc.url == TestRTCBase.valid_test_urls[idx]
            assert test_rtc["url"] == TestRTCBase.valid_test_urls[idx]
