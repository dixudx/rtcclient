...
class RTCClient(RTCBase):
    ...
    def _get_headers(self):
        ...
        resp = self.get(
            self.url + "/authenticated/identity",
    ...
    def getProjectAreas(self, archived=False, returned_properties=None):
        ...
        return self._getProjectAreas(
            archived=archived, returned_properties=returned_properties
        )

    def getProjectArea(
        self, projectarea_name, archived=False, returned_properties=None
    ):
        ...
        proj_areas = self._getProjectAreas(
            archived=archived, returned_properties=rp, projectarea_name=projectarea_name
        )
    ...
    def _getProjectAreas(
        ...
        return self._get_paged_resources(
            "ProjectArea",
            page_size="10",
            archived=archived,
    ...
    def getProjectAreaByID(
        self, projectarea_id, archived=False, returned_properties=None
    ):
        ...
        rp = returned_properties
        proj_areas = self._getProjectAreas(
            archived=archived, returned_properties=rp, projectarea_id=projectarea_id
        )
        if proj_areas is not None:
            proj_area = proj_areas[0]
            self.log.info("Find <ProjectArea %s>", proj_area)
    ...
    def checkProjectAreaID(self, projectarea_id, archived=False):
        ...
        self.log.debug("Check the validity of the ProjectArea id: %s", projectarea_id)

        proj_areas = self._getProjectAreas(
            archived=archived, projectarea_id=projectarea_id
        )
        if proj_areas is not None:
            proj_area = proj_areas[0]
            self.log.info(
                "Find <ProjectArea %s> whose id is: %s", proj_area, projectarea_id
            )
            return True
    ...
    def _getTeamAreas(
        ...
        projarea_id = self._pre_get_resource(
    ...
    def getOwnedBy(self, email, projectarea_id=None, projectarea_name=None):
        ...
        new_parse_result = urlparse.ParseResult(
    ...
    def getPlannedFor(
        ...
        if not isinstance(plannedfor_name, six.string_types) or not plannedfor_name:
        ...
        rp = returned_properties
        plannedfors = self._getPlannedFors(
            projectarea_id=projectarea_id,
            projectarea_name=projectarea_name,
            archived=archived,
            returned_properties=rp,
    ...
    def getSeverity(self, severity_name, projectarea_id=None, projectarea_name=None):
        ...
        severities = self._getSeverities(
            projectarea_id=projectarea_id,
            projectarea_name=projectarea_name,
    ...
    def getSeverities(self, projectarea_id=None, projectarea_name=None):
        ...
        return self._getSeverities(
            projectarea_id=projectarea_id, projectarea_name=projectarea_name
        )

    def _getSeverities(
        self, projectarea_id=None, projectarea_name=None, severity_name=None
    ):
        projarea_id = self._pre_get_resource(
            projectarea_id=projectarea_id, projectarea_name=projectarea_name
        )
        if projarea_id is None:
            self.log.error(
                "Please input either-or between " "projectarea_id and projectarea_name"
            )
            raise exception.EmptyAttrib(
                "At least input either-or between "
    ...
    def getPriority(self, priority_name, projectarea_id=None, projectarea_name=None):
    ...
    def getPriorities(self, projectarea_id=None, projectarea_name=None):
        ...
        return self._getPriorities(
            projectarea_id=projectarea_id, projectarea_name=projectarea_name
        )

    def _getPriorities(
        self, projectarea_id=None, projectarea_name=None, priority_name=None
    ):
        projarea_id = self._pre_get_resource(
            projectarea_id=projectarea_id, projectarea_name=projectarea_name
        )
    ...
    def getFoundIn(
        self, foundin_name, projectarea_id=None, projectarea_name=None, archived=False
    ):
    ...
    def getFoundIns(self, projectarea_id=None, projectarea_name=None, archived=False):
    ...
    def getFiledAgainst(
        ...
        if not isinstance(filedagainst_name, six.string_types) or not filedagainst_name:
            excp_msg = "Please specify a valid FiledAgainst name"
            self.log.error(excp_msg)
            raise exception.BadValue(excp_msg)

        fas = self._getFiledAgainsts(
            projectarea_id=projectarea_id,
            projectarea_name=projectarea_name,
            archived=archived,
    ...
    def getFiledAgainsts(
        self, projectarea_id=None, projectarea_name=None, archived=False
    ):
        ...
        return self._getFiledAgainsts(
            projectarea_id=projectarea_id,
            projectarea_name=projectarea_name,
            archived=archived,
        )

    def _getFiledAgainsts(
        self,
        projectarea_id=None,
        projectarea_name=None,
        archived=False,
    ...
    def getTemplate(
        ...
        return self.templater.getTemplate(
            copied_from,
    ...
    def getWorkitem(self, workitem_id, returned_properties=None):
        ...
        try:
            ...
            if rp is not None:
                req_url = "".join([workitem_url, "?oslc_cm.properties=", urlquote(rp)])
            else:
                req_url = workitem_url
            resp = self.get(
                req_url, verify=False, proxies=self.proxies, headers=self.headers
            )
            raw_data = xmltodict.parse(resp.content)
            workitem_raw = raw_data["oslc_cm:ChangeRequest"]

            return Workitem(
                workitem_url, self, workitem_id=workitem_id, raw_data=workitem_raw
            )
    ...
    def getWorkitems(
        ...
        if not isinstance(projectarea_id, six.string_types) or not projectarea_id:
            ...
            if pa_ids is None:
                self.log.warning("Stop getting workitems because of " "no ProjectAreas")
                return None
            ...
            if self.checkProjectAreaID(projectarea_id):
                ...
                raise exception.BadValue(
                    "Invalid ProjectAred ID: " "%s" % projectarea_id
                )

        self.log.warning(
            "For a single ProjectArea, only latest 1000 "
        ...
        if not workitems_list:
            self.log.warning(
                "Cannot find a workitem in the ProjectAreas "
    ...
    def createWorkitem(
        ...
        **kwargs
        ...
        if not template:
            if not copied_from:
                self.log.error(
                    "Please choose either-or between " "template and copied_from"
                )
                raise exception.EmptyAttrib(
                    "At least choose either-or " "between template and copied_from"
                )

            self._checkMissingParamsFromWorkitem(copied_from, keep=keep, **kwargs)
            kwargs = self._retrieveValidInfo(projectarea_id, **kwargs)
            wi_raw = self.templater.renderFromWorkitem(
                copied_from,
                keep=keep,
                encoding="UTF-8",
                title=title,
            ...
            kwargs = self._retrieveValidInfo(projectarea_id, **kwargs)
            wi_raw = self.templater.render(
                template, title=title, description=description, **kwargs
            )

        self.log.info("Start to create a new <%s> with raw data: %s", item_type, wi_raw)
    ...
    def copyWorkitem(self, copied_from, title=None, description=None, prefix=None):
        ...
        self.log.info(
            "Start to create a new <Workitem>, copied from " "<Workitem %s>",
            copied_from,
        )
    ...
    def _retrieveValidInfo(self, projectarea_id, **kwargs):
        ...
        for keyword in kwargs.keys():
            try:
                ...
                keyword_obj = keyword_cls(
                    kwargs[keyword], projectarea_id=projectarea_id
                )
    ...
    def _get_paged_resources(
        ...
        self.log.debug(
            "Start to fetch all %ss with [ProjectArea ID: %s] " "and [archived=%s]",
            resource_name,
        ...
        res_map = {
            ...
            "Administrator": "/".join(
                ["projectareas", "%s" % projectarea_id, "rtc_cm:administrators"]
            ),
            ...
            "Subscriber": "/".join(
                ["workitems", "%s" % workitem_id, "rtc_cm:subscribers"]
            ),
            "Action": "workflows/%s/actions/%s" % (projectarea_id, customized_attr),
            "Query": "".join(
                [
            ...
            "State": "workflows/%s/states/%s" % (projectarea_id, customized_attr),
            "SavedQuery": "queries",
            "RunQuery": "queries/%s/rtc_cm:results" % customized_attr,
            "IncludedInBuild": "workitems/%s/%s" % (workitem_id, customized_attr),
            "Parent": "workitems/%s/%s" % (workitem_id, customized_attr),
            "Children": "workitems/%s/%s" % (workitem_id, customized_attr),
            "ChangeSet": "workitems/%s/%s" % (workitem_id, customized_attr),
            "Attachment": "workitems/%s/%s" % (workitem_id, customized_attr),
        }
        ...
        if returned_properties is not None:
            if not isinstance(returned_properties, six.string_types):
                raise exception.BadValue("returned_properties is not a" "valid string")
            resource_url = "".join(
                [resource_url, "&oslc_cm.properties=", urlquote(returned_properties)]
            )

        pa_url = (
            "/".join([self.url, "oslc/projectareas", projectarea_id])
            if projectarea_id
            else None
        )

        resp = self.get(
            resource_url, verify=False, proxies=self.proxies, headers=self.headers
        )
        raw_data = xmltodict.parse(resp.content)
        ...
        if not resources_list:
            self.log.warning(
                "No %ss are found with [ProjectArea ID: %s] " "and [archived=%s]",
                resource_name,
                projectarea_id if projectarea_id else "not specified",
                archived,
            )
    ...
    def _handle_resource_entry(
        ...
        if projectarea_url is not None:
            try:
                if (
                    entry.get("rtc_cm:projectArea").get("@rdf:resource")
                ) != projectarea_url:
                    return None
        ...
        if entry_archived is not None and eval(entry_archived.capitalize()) != archived:
            return None
        ...
        if resource_name in ["Workitem", "Query", "RunQuery", "Parent", "Children"]:
            resource_url = entry.get("@rdf:resource")
            resource_url = "/".join(
                [self.url, "oslc/workitems", resource_url.split("/")[-1]]
            )
        else:
            resource_url = entry.get("@rdf:resource")
    ...
    def queryWorkitems(
        ...
        rp = returned_properties
        return self.query.queryWorkitems(
            query_str=query_str,
            projectarea_id=projectarea_id,
            projectarea_name=projectarea_name,
            returned_properties=rp,
